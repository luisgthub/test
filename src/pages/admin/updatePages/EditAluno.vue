          ...
          <div class="px-4 py-5 bg-white sm:p-6">
            <div class="grid grid-cols-6 gap-6">
              <div class="col-span-6 sm:col-span-3">
                ...
                <input v-model="data.first_name" type="text"  autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md " placeholder="">
                   <span class="font-mono text-red-700">{{ firstNameError }}</span>
              </div>

              <div class="col-span-6 sm:col-span-3">
                <label for="last-name" class="block text-sm font-medium text-gray-700">Last name</label>
                <input v-model="data.last_name" type="text" name="last-name" id="last-name" autocomplete="family-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                 <span class="font-mono text-red-700">{{ lastNameError }}</span>
              </div>

              <div class="col-span-6 sm:col-span-4">
                <label for="email-address" class="block text-sm font-medium text-gray-700">Email address</label>
                <input v-model="data.email" type="text" name="email-address" id="email-address" autocomplete="email" class=" peer mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
      ...
      const data = reactive({
        ...
        onMounted(async ()=>{
          ...
          roles.value=rolesResponse.data;
            ...
            const edit = await axios.get(`users/${route.params.id}`);
            data.first_name = edit.data.first_name;
            data.last_name = edit.data.last_name;
            data.email = edit.data.email;
            data.role_id = edit.data.role.id;
